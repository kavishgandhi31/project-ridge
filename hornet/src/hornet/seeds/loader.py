...
def _read_seed_file(filename: str) -> str:
    """Read a YAML seed file from the packaged ``hornet.seeds.data`` resources.
    ...
    try:
        ...
        raise FileNotFoundError(
            ...
            f"Is the hornet package installed correctly? "
            f"Expected at: {ref}"
        ) from None
...
async def seed_all(
    *,
    ...
    ``hornet.seeds.data``. Tests pass a Path to exercise edge cases
